...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${CORE_API_URL}/api/notifications/logs/trend?range=${encodeURIComponent(range)}`,
      {
        credentials: 'include',
        headers: {
    ...
    logger.error('[proxy] Error obteniendo tendencia de notificaciones:', error)
    return NextResponse.json(
      { success: false, message: error?.message || 'Error interno en proxy trend' },
      { status: 500 }
    )
  }
}
