...
export async function POST(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const response = await fetch(
      `${CORE_API_URL}/api/notifications/campaigns/${params.id}/execute`,
      {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            json?.message || text || response.statusText || 'No se pudo ejecutar la campaña',
        },
    ...
    logger.error('[proxy] Error ejecutando campaña:', error)
    return NextResponse.json(
